    ...
    def train(self, dataset):
        ...
        while (accuracy!=1.0):
            ...
            for x, y in dataset.iterate_once(batchsize):
                label = self.get_prediction(x)
                trueLabel = nn.as_scalar(y)
                totalCount += 1
                if (label!=trueLabel):
                    weights = self.get_weights()
                    weights.update(x, trueLabel)
                else:
                    scoreCount += 1
            accuracy = scoreCount / totalCount
...
class RegressionModel(object):
    ...
    def __init__(self):
        ...
        for i in range(self.layers):
            if (i==0):
                self.m.append(nn.Parameter(1, self.size))
                self.b.append(nn.Parameter(self.batch_size, self.size))
            elif (i==self.layers-1):
                self.m.append(nn.Parameter(self.size, 1))
    ...
    def run(self, x):
        ...
        for i in range(0, self.layers):
            m = self.m[i]
            b = self.b[i]
            xm = nn.Linear(predictedY, m)
            predictedY = nn.AddBias(xm, b)
            if (i<self.layers-1):
                predictedY = nn.ReLU(predictedY)
        return predictedY
    ...
    def train(self, dataset):
        ...
        while (numLoss>0.02):
            for x, y in dataset.iterate_once(batch_size):
                ...
                if (numLoss>0.02):
                    mixedGrad = nn.gradients(loss, self.m+self.b)
                    length = len(mixedGrad)
                    for i in range(0, self.layers):
                        self.m[i].update(mixedGrad[i], -self.multiplier)
                        self.b[i].update(mixedGrad[i+length//2], -self.multiplier)
    ...
    def __init__(self):
        ...
        for i in range(self.layers):
            if (i==0):
                ...
                self.b.append(nn.Parameter(self.batch_size, self.output_dimension))
            else:
                self.m.append(nn.Parameter(self.size, self.size))
                self.b.append(nn.Parameter(self.batch_size, self.size))
